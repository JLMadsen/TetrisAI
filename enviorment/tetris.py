...
class Tetris():
    ...
    def discretization(self):
        ...
        piece_layer = [[0 for _ in range(self.game_columns)]
                       for _ in range(self.game_rows)]
    ...
    def reset(self):
        self.state = [[0 for _ in range(self.game_columns)]
                      for _ in range(self.game_rows)]
        ...
        self.current_shape = self.get_blocks_from_shape(
            shape1, self.current_piece, self.start_position)
        self.next_shape = self.get_blocks_from_shape(
            shape2, self.next_piece, self.start_position)
    ...
    def step(self, action):
        ...
        if action == Action.DOWN:
            ...
            self.current_rotation = (
                self.current_rotation - 1) % len(self.shapes.ALL[self.current_piece])
            new_rotation = self.shapes.ALL[self.current_piece][self.current_rotation]
            next_position = self.get_blocks_from_shape(
                new_rotation, self.current_piece, self.current_shape[0])
            for y, x in next_position:
        ...
        if placed:
            ...
            self.next_shape = self.get_blocks_from_shape(
                shape, self.next_piece, self.start_position)
    ...
    def render(self, manual=0):
        ...
        for block in temp_shape:
            ...
            color = tuple([c-200 if c > 200 else c-100 if c >
                           100 else 0 for c in color])
        ...
        for block in self.current_shape:
            ...
            pg.draw.rect(
                self.screen, self.shapes.COLORS[self.current_piece], rect, 0)
        ...
        for block in self.next_shape:
            ...
            rect = pg.Rect(next_preview[0] + (block[1] - 2 + center_x) * self.cell_size,
                           next_preview[1] + (block[0] + 1 +
                                              center_y) * self.cell_size,
                           self.cell_size,
                           self.cell_size)

            pg.draw.rect(
                self.screen, self.shapes.COLORS[self.next_piece], rect, 0)

        score_text = self.font.render(
            ("Score: " + str(self.score)), 1, Color.WHITE)
        score_textRect = score_text.get_rect()
        score_textRect.center = (self.info_margin_left, 200)

        highscore_text = self.font.render(
            ("Highscore: " + str(self.highscore)), 1, Color.WHITE)
        highscore_textRect = highscore_text.get_rect()
        highscore_textRect.center = (self.info_margin_left, 240)

        attempt_text = self.font.render(
            ("Attempts: " + str(self.attempt)), 1, Color.WHITE)
    ...
    def __initView(self):
        ...
        self.background = pg.image.load(
            str(mod_path) + '/sprites/background.png')
        self.background = pg.transform.scale(
            self.background, (self.window_height, self.window_width))
    ...
    def get_all_states(self, display=True):
        ...
        for r in range(1, len(self.shapes.ALL[self.current_piece]) + 1):
            for i in range(self.game_columns):
                ...
                traverse = self.actions.LEFT if (
                    i-(self.game_columns//2)) < 0 else self.actions.RIGHT
                actions[-1] += [traverse] * \
                    (i if i < (self.game_columns//2)
                     else i-(self.game_columns//2)+1)
    ...
    def heuristic_value(self, state):
        ...
        evenness = sum([i for i in [abs(heights[-1]-heights[j])
